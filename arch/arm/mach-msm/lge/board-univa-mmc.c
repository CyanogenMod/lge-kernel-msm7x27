/* arch/arm/mach-msm/lge/board-univa-mmc.c
...
#ifdef CONFIG_MMC_MSM_CARD_HW_DETECTION
static unsigned int univa_sdcc_slot_status(struct device *dev)
{
#if 1 /* FEATURE_LGE_FS, warkap */
	unsigned int mmc_slot_state;
	mmc_slot_state = (unsigned int)gpio_get_value(sd_detect_gpio);
	return !mmc_slot_state;
#else
