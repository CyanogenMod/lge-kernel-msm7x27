/* arch/arm/mach-msm/lge/board-muscat-pm.c
...
int muscat_pwrsink_suspend_noirq(struct device *dev)
{
...
int muscat_pwrsink_resume_noirq(struct device *dev)
{
...
static struct dev_pm_ops muscat_pwrsink_data = {
	.suspend_noirq = muscat_pwrsink_suspend_noirq,
	.resume_noirq = muscat_pwrsink_resume_noirq,
};

static struct platform_device muscat_pwrsink_device = {
	.name = "lge-pwrsink",
	.id = -1,
	.dev = {
		.platform_data = &muscat_pwrsink_data,
	},
};
...
{
	lcd_bl_power_state = BL_POWER_RESUME;
	platform_device_register(&muscat_pwrsink_device);
}
