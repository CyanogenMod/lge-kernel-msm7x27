...
static void  evrc_in_get_dsp_frames(struct q6audio_evrc *audio,
		uint32_t token,	uint32_t *payload);
static int evrc_in_wait_for_eos_ack(struct q6audio_evrc *audio);
static int evrc_in_flush(struct q6audio_evrc *ac);
static int evrc_in_pause(struct q6audio_evrc *ac);

void q6asm_evrc_in_cb(uint32_t opcode, uint32_t token,
		uint32_t *payload, void *priv)
{
	struct q6audio_evrc * audio = (struct q6audio_evrc *)priv;
	unsigned long flags;
	...
	case ASM_DATA_EVENT_READ_DONE:
		evrc_in_get_dsp_frames(audio, token, payload);
		break;
...
static int evrc_in_enable(struct q6audio_evrc  *audio)
{
	if (audio->enabled)
		return 0;
...
static int evrc_in_flush(struct q6audio_evrc  *audio)
{
	int rc;

	MM_DBG("flush\n");
	/* Implicitly issue a pause to the decoder before flushing */
	rc = evrc_in_pause(audio);
	if (rc < 0) {
...
static int evrc_in_pause(struct q6audio_evrc  *audio)
{
	int rc;
	...
	if (evrc_in_wait_for_eos_ack(audio) < 0) {
...
static int evrc_buf_alloc(struct q6audio_evrc *audio)
{
	int rc = 0;
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_START: {
		...
		rc = q6asm_enc_cfg_blk_evrc(audio->ac,
			...
			audio->enc_cfg.max_bit_rate, 0);

		if (rc < 0) {
			MM_ERR("cmd evrc media format block failed\n");
			break;
		}
		if (audio->pcm_feedback == NON_TUNNEL_MODE) {
	...
	case AUDIO_SET_STREAM_CONFIG: {
		...
		if ((cfg.buffer_size < (23+sizeof(struct meta_out_dsp))) ||
...
static ssize_t evrc_in_read(struct file *file,
				char __user *buf,
...
static ssize_t evrc_in_write(struct file *file,
...
static int evrc_in_release(struct inode *inode, struct file *file)
{
	struct q6audio_evrc  *audio = file->private_data;
	MM_INFO("\n");
	mutex_lock(&audio->lock);
...
{
	struct q6audio_evrc *audio = NULL;
	int rc = 0;

	audio = kzalloc(sizeof(struct q6audio_evrc), GFP_KERNEL);
...
static const struct file_operations audio_in_fops = {
	.owner		= THIS_MODULE,
	.open		= evrc_in_open,
	.release	= evrc_in_release,
	.read		= evrc_in_read,
	.write		= evrc_in_write,
	.unlocked_ioctl	= evrc_in_ioctl,
};
